...
// Shared state exported so main.js / drone.js can read / modify them
export let deckLights = [];
export let ambientParticles = [];
export let obstacleData = {
  columns: [],  // {position, radius}
  rings: [],    // {position, radius, normal, scored, mesh}
};

/**
 * Resets module-level arrays to a clean state (called on resetSimulation).
 * This ensures re-running setup after a reset doesn't double-add.
 */
export function resetSceneData() {
  deckLights = [];
  ambientParticles = [];
  obstacleData = { columns: [], rings: [] };
}

/**
 * Advanced Lighting Topology Array
 */
export function setupLightingArray(scene) {
 ...
 */
export function setupLabEnvironment(scene) {
  // ===== FLOOR =====
  const size = 512;
  ...
  barrierPositions.forEach(bp => {
    const wall = new THREE.Mesh(new THREE.PlaneGeometry(bp.sx, 0.5), barrierMat);
 ...
 */
export function setupObstacleCourse(scene) {
  ...
  // Target positions around the arena
  ...
  columnPositions.forEach(pos => {
    ...
    columnGroup.position.set(pos.x, 4, pos.z);
    ...
    // Double glowing neon bands around pillars
    const bandGeo = new THREE.CylinderGeometry(1.05, 1.05, 0.2, 6);
    const band1 = new THREE.Mesh(bandGeo, neonRingMat);
    band1.position.y = 2;
    columnGroup.add(band1);
    const band2 = new THREE.Mesh(bandGeo, neonRingMat);
    band2.position.y = -2;
    columnGroup.add(band2);
...
/**
 * Animates rings (rotation + bobbing) — called from main loop
 */
export function updateObstacleAnimations(dt, time) {
